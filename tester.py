...
def test(epoch,model,loader,TRAIN_EPOCHS,IMAGE_SHOW_TRESHOLD,train_func,device):
    global total_metric_min
    model.eval()
    correct = 0

    for data in loader:
        for epoch_train in range(1, TRAIN_EPOCHS):
            train_func(0,data=data)
        data = data.to(device)
...
def test_one_hot(epoch,model,loader,TRAIN_EPOCHS,IMAGE_SHOW_TRESHOLD,train_func,device):
    global total_metric_min
    model.eval()
    correct = 0
